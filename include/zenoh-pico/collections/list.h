...
_z_list_t *_z_list_extract_filter(_z_list_t *xs, void *found, z_element_eq_f c_f, void *left);
static inline _z_list_t *_z_list_drop_filter(_z_list_t *xs, z_element_free_f f_f, z_element_eq_f c_f, void *left) {
    void *found = NULL;
    _z_list_t *l = _z_list_extract_filter(xs, found, c_f, left);
    if (found != NULL) {
        f_f(found);
    }
    return l;
...
#define _Z_LIST_DEFINE(name, type)                                                                                     \
    ...
    static inline name##_list_t *name##_list_extract_filter(name##_list_t *l, type *found, name##_eq_f c_f, type *e) { \
        return _z_list_extract_filter(l, found, (z_element_eq_f)c_f, e);                                               \
    }                                                                                                                  \
